...
class CommandCenterTests(unittest.TestCase):
    def test_if_motors_and_sensors_are_properly_identified(self):
        motor = PluginDetails("motor_name", "motor_key", Motor(), "motor_path")
        sensor = PluginDetails("sensor_name", "sensor_key", Sensor(), "sensor_path")
    ...
    def test_if_only_unique_keys_are_inserted_all_others_are_ignored(self):
        motor1 = PluginDetails("motor_name", "motor_key", Motor(), "motor_path")
        motor2 = PluginDetails("motor_name", "motor_key", Motor(), "motor_path")
